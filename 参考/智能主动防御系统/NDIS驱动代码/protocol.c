...
#pragma hdrstop
//基本过滤规则
extern UCHAR ICMP_IN_ALLOW; //是否允许Ping入
...
--*/
{
    ...
    else do
    {
        ...
        if (Packet!=NULL)
        {
        	  int flag=0;
        	  //数据包过滤
        	  DBGPRINT(("PtReceive 数据包过滤!"));
        	  NdisAcquireSpinLock(&PacketLock);
        	  if(PACKET_FILTER) flag=1;
        	  NdisReleaseSpinLock(&PacketLock);  	
        	  if(flag)
        	     if(FilterPacket(Packet)==NDIS_STATUS_NOT_ACCEPTED) return NDIS_STATUS_NOT_ACCEPTED;
            //
